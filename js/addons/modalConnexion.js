module.exports=function($q,$compile,$rootScope,$sce){
    ...
    this.showModal=function(title,content,then){
    	...
    	//}
    	this.scope.buttons=[{caption:"Connexion",dismiss:"true"}];
    	this.scope.then=then;
    	this.scope.title=title;
    	this.scope.content=$sce.trustAsHtml(content);
    	var elm=angular.element('<bs-modal did="id-dialog" on-exit="then(button)" buttons="buttons" show="showDialog" title="{{title}}" little="true"><div data-ng-bind-html="content"></div></bs-modal>');
